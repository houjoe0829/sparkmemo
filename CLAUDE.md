...
**Spark Memo** is an Obsidian plugin for journaling. It highlights and manages `HH:MM` timestamps in a designated Memo section, provides a quick-capture sidebar (record / search / tags / locations / stats) for jotting down text, images, and voice recordings straight into today's daily note, plots every geo-tagged memo on a bundled offline world map, and includes a multi-language (中文/English) UI that follows Obsidian's configured language.
...
- **`capture-view.ts`** — The quick-capture sidebar (`JournalCaptureView`, ~4000 lines). A single `ItemView` with four tab panes:
  ...
  - **地点 (Locations)** — two views over entries carrying a `[name](geo:lat,lon)` tag: an offline world map (default) and a by-city list.
...
- **`map-view.ts`** — Pure geometry layer for the map view (Obsidian-free, like `stats.ts`): Web Mercator projection clipped to 78N..60S, single-pass greedy clustering in screen pixels, dot sizing, and time-zone-derived initial longitude. There is no zoom, so exactly one clustering result is alive at a time.
- **`land-data.ts`** — The bundled basemap: Natural Earth 1:50m land outline (public domain), Visvalingam-simplified and pre-projected to an integer grid, emitted as one SVG path string. Generated offline and committed as data — no build-time dependency, no runtime network request, and no national borders (land/sea only).
- **`webp-encoder.ts`** — WASM WebP encoder wrapper (`@jsquash/webp`), used because Safari/iOS can't encode WebP via `canvas.toBlob`.
- **`chat-runtime.ts`** — Drives the locally installed `claude` CLI as a subprocess and turns its NDJSON stream into `ChatChunk`s. **The only module that touches Node**, and desktop-only — see the mobile-safety rules below.
- **`chat-store.ts`** — Conversation persistence, one JSON per thread under the vault's `.spark-memo/chats/`. Obsidian-free like `stats.ts`; takes a `ChatStorageAdapter` that `vault.adapter` structurally satisfies.
- **`chat-pane.ts`** — The chat tab's UI: conversation list, thread view, streaming renderer, composer. See [docs/specs/chat.md](docs/specs/chat.md).
...
**Mobile Safety (chat only):** Obsidian mobile has no `child_process`. A top-level import of it would fail at plugin load and take *the entire plugin* down on mobile, not just chat. Three rules keep that from happening, and all three must hold together:
1. `chat-runtime.ts` requires Node modules via `require()` **inside function bodies**, never as top-level imports.
2. `scripts/build.js` lists `fs` / `os` / `path` / `child_process` as esbuild `external`, so those `require` calls survive verbatim instead of being bundled or hoisted.
3. `capture-view.ts` reaches the runtime through a dynamic `import()` behind `Platform.isDesktopApp`.

After changing any of this, verify the bundle: every `require(` must sit inside a function, and `init_chat_runtime` must stay lazily invoked.
...
- **`src/map-view.ts`** — Map projection, clustering, dot sizing (Obsidian-free).
- **`src/land-data.ts`** — Bundled world basemap path data.
- **`src/webp-encoder.ts`** — WASM WebP encoder wrapper.
- **`src/chat-runtime.ts`** — `claude` CLI subprocess + NDJSON stream parsing (desktop only, the sole Node consumer).
- **`src/chat-store.ts`** — Conversation persistence (Obsidian-free).
- **`src/chat-pane.ts`** — Chat tab UI.
...
- **Mobile keyboard inset:** when the software keyboard opens, Obsidian both shrinks the leaf to the top of the keyboard *and* adds an inline `padding-bottom` the height of the keyboard to `.view-content` — which is our capture root. Since the root is a fixed-height flex column rather than a scrolling view, that inset left ~4px of usable height and collapsed the whole timeline. `.is-mobile .jp-capture-root` overrides it back to `0 !important`. Keep this in mind before touching the sidebar's vertical layout; see [docs/bugs](docs/bugs/) for the full trace.
