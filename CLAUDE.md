...
- **`myclaudian-bridge.ts`** — Resolves the MyClaudian plugin instance, duck-typing `sendFromPlugin` rather than trusting the plugin id alone (an older MyClaudian is installed but cannot be sent to).
- **`send-to-myclaudian-modal.ts`** — The "Send to MyClaudian" composer: memo preview, instruction box, one-tap suggestions. See [docs/specs/send-to-myclaudian.md](docs/specs/send-to-myclaudian.md).
...
- **`src/myclaudian-bridge.ts`** — Detects the MyClaudian plugin (desktop only).
- **`src/send-to-myclaudian-modal.ts`** — "Send to MyClaudian" composer dialog.
...
| `sendToMyClaudianEnabled` | boolean | `false` | Master switch for the "Send to MyClaudian" right-click entry. Deliberately absent from the settings tab — turning it on means editing the default and rebuilding, since it only works next to a locally installed MyClaudian |
