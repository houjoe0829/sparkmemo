...
- **粘贴图片**：在输入框内直接 `⌘V` 粘贴剪贴板里的图片，插件自动保存到 Vault 附件目录，并以缩略图卡片的形式显示在按钮组右侧（暂不插入正文）；也支持把图片文件**拖拽**到输入框。每次操作仅处理一张图片，可重复添加，点击缩略图上的 × 可移除
- **图片按钮**：点击输入框下方的图片图标，从本地选择图片添加为待发送的缩略图
- **提交时合并**：点击 NOTE 提交时，所有待发送的图片会以 `![[path]]` 嵌入语法统一追加到正文末尾
