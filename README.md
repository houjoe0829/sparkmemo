...
- **图片拍摄时间校正**：粘贴/上传的图片若携带的拍摄时间（EXIF 或文件修改时间）与当前时间相差较大，会询问是否改用图片时间记录这条 memo；确认后「+」按钮右侧会出现一个时间胶囊，可随时点击 × 改回使用当前时间
...
| 设置项 | 说明 | 默认值 |
|---|---|---|
| 图片压缩 | 添加图片时自动压缩后再存入 Vault（非 PNG 统一转为 WebP，iOS 因系统限制会转为 JPEG）；GIF 不受影响（保留动画） | 开启 |
| 压缩质量 | WebP 重新编码的质量（0.1–1.0），PNG 无损不受影响 | `0.8` |
